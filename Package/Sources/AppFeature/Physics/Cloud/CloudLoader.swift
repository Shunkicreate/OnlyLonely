...
class CloudLoader {
    ...
    /// ランダムに雲データを生成して配置（レーン単位）
    @MainActor
    static func loadLaneClouds(
        lane: PlayerSlot,
        laneSize: CGSize,
        cloudsPerLane: Int = 5,
        ...
        var laneCloudData: [CloudData] = []
        var cloudNodes: [Int: SKNode] = [:]

        guard laneSize.width > 0, laneSize.height > PhysicsConstants.groundBaseline else {
            physicsCoordinator?.registerCloudData([], for: lane)
            return [:]
        }

        let baseId = lane == .playerA ? 1000 : 2000
        let xPadding: CGFloat = 40
        let xRange = xPadding...(laneSize.width - xPadding)
        let minY = PhysicsConstants.groundBaseline + 160
        let maxY = max(minY + 40, laneSize.height - 120)

        for index in 0..<cloudsPerLane {
            let type = cloudTypes.randomElement() ?? .ume
            ...
            while cloudData == nil && attempts < maxAttempts {
                ...
                let randomWidth = CGFloat.random(in: laneSize.width * 0.35...laneSize.width * 0.65)
                let randomHeight = CGFloat.random(in: 40...80)

                let candidate = CloudData(
                    id: baseId + index,
                    type: type,
                    position: CGPoint(x: randomX, y: randomY),
                    size: CGSize(width: randomWidth, height: randomHeight),
                    lightningInterval: type == .matsu ? Double.random(in: 2.5...4.0) : nil,
                    speedThreshold: type == .take ? CGFloat.random(in: 40...60) : nil
                )

                if !isOverlapping(candidate, with: laneCloudData) {
                    cloudData = candidate
                }
                attempts += 1
            }
            ...
            if let data = cloudData {
                laneCloudData.append(data)
                let node = createCloudNode(cloudData: data)
                node.position = data.position
                node.zPosition = 5
                scene.addChild(node)
                cloudNodes[data.id] = node
            }
        }

        physicsCoordinator?.registerCloudData(laneCloudData, for: lane)
        return cloudNodes
    }
